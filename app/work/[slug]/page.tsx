    ...
    <>
      ...
      <div className="border-b border-border/40">
        <div className="container mx-auto max-w-4xl px-6 py-16 md:py-24">
          ...
          <h1 className="mt-6 text-5xl font-normal tracking-tight md:text-6xl lg:text-7xl">{caseStudy.title}</h1>
          <p className="mt-6 text-xl text-muted-foreground leading-relaxed">{caseStudy.deck}</p>

          <div className="mt-8 flex flex-wrap gap-2">
            {caseStudy.stack.map((tech) => (
              <span key={tech} className="rounded-md bg-muted px-3 py-1 text-sm font-medium text-muted-foreground">
                {tech}
              </span>
            ))}
          </div>
      ...
      <article className="container mx-auto max-w-4xl px-6 py-16">
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-8">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">TL;DR</h2>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {caseStudy.metrics.map((metric) => (
              <MetricChip key={metric.label} label={metric.label} value={metric.value} variant="accent" />
            ))}
          </div>
        </div>
        ...
        <section id="overview" className="mt-16">
          ...
          <p className="mt-4 text-base md:text-lg leading-relaxed text-muted-foreground">{caseStudy.overview}</p>
        </section>

        {/* Problem */}
        <section id="problem" className="mt-16">
          <h2 className="text-3xl font-normal md:text-4xl">Problem</h2>
          <p className="mt-4 text-base md:text-lg leading-relaxed text-muted-foreground">{caseStudy.problemDetails}</p>
        </section>

        {/* Approach */}
        <section id="approach" className="mt-16">
          <h2 className="text-3xl font-normal md:text-4xl">Approach</h2>
          <p className="mt-4 text-base md:text-lg leading-relaxed text-muted-foreground">{caseStudy.approach}</p>
        </section>
        ...
        <section id="architecture" className="mt-16">
          ...
          <div className="mt-6 rounded-lg border border-border bg-muted/20 p-8">
            <div className="text-base md:text-lg leading-relaxed text-muted-foreground">{caseStudy.architecture}</div>
            
            {slug === "bigquery-rerun-manager" && (
              <div className="mt-8 space-y-8">
                {/* High-Level Architecture */}
                <div>
                  <h3 className="text-lg font-normal mb-4 text-foreground">High-Level Architecture</h3>
                ...
                {/* Execution Flow */}
                <div>
                  <h3 className="text-lg font-normal mb-4 text-foreground">Execution Flow</h3>
                  <div className="rounded-lg border border-border bg-background p-6">
                    <Image 
                ...
                {/* Data Model */}
                <div>
                  <h3 className="text-lg font-normal mb-4 text-foreground">Data Model</h3>
                  <div className="rounded-lg border border-border bg-background p-6">
                    <Image 
          ...
          <Accordion type="single" collapsible className="mt-6">
            {caseStudy.decisions.map((decision, index) => (
              <AccordionItem key={index} value={`decision-${index}`}>
                <AccordionTrigger className="text-lg font-semibold">{decision.title}</AccordionTrigger>
        ...
        <section id="results" className="mt-16">
          ...
          <p className="mt-4 text-base md:text-lg leading-relaxed text-muted-foreground">{caseStudy.results}</p>
          ...
          <div className="mt-8 flex h-64 items-center justify-center rounded-lg border border-border bg-muted/20">
        ...
        <section id="learnings" className="mt-16">
          ...
          <p className="mt-4 text-base md:text-lg leading-relaxed text-muted-foreground">{caseStudy.learnings}</p>

          <div className="mt-8 rounded-lg border border-border bg-accent/30 p-6">
            <h3 className="font-semibold">What I'd Do Next</h3>
            <p className="mt-2 text-muted-foreground leading-relaxed">{caseStudy.nextSteps}</p>
          </div>
        </section>

        {/* Next Case Study */}
        <div className="mt-24 border-t border-border pt-12">
          <p className="text-sm uppercase tracking-wider text-muted-foreground">Next Case Study</p>
          <Link
          ...
          >
            <div>
              ...
              <p className="mt-2 text-muted-foreground">{nextProject.problem}</p>
            </div>
